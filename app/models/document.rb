class Document < ActiveRecord::Base
  ...
  DOCUMENTS_TERSE = {
    ...
    'documents.state' => 'documents.state',
    'documents.checked_out_by' => 'documents.checked_out_by',
    'documents.document_type_id' => 'documents.document_type_id'
  }
  ...
  scope :simple, lambda{ |view, text|
    ...
    columns=DOCUMENTS_TERSE.clone

    unless view.nil?
      view.cell_definitions.each do |cell|
        column = "#{cell.table_name}.#{cell.column_name}"
        unless columns.include?(column)
          if cell.table_name == 'documents'
            columns[column] = column
          end
        end
      end
    ...
    select(columns.values.join(',')).where(sanitize_sql_array(["MATCH (documents.body, documents.metadata, documents.custom_metadata) AGAINST ('%s' IN BOOLEAN MODE)", text.gsub("'", "''")]))
  }
